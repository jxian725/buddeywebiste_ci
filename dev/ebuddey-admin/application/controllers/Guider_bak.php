...
class Guider extends CI_Controller {
	...
	public function index() {
        ...
        $data[ 'Emessage' ]                     = '';
        $data[ 'Smessage' ]                     = '';
        $data[ 'header' ][ 'title' ]            = 'Host lists';
        $data[ 'header' ][ 'metakeyword' ]      = 'Host lists';
        $data[ 'header' ][ 'metadescription' ]  = 'Host lists';
        $data[ 'footer' ][ 'script' ]           = $script;
        $data[ 'content' ]                      = $content;
        $data[ 'breadcrumb' ]                   = '<li class="active">Host lists</li>';
        $this->template( $data );
	}
    public function view()
    {
        ...
        if($field == 'profile_image' || $field == 'id_proof') {
            $guider_id  = $this->input->post( 'guider_id' );
            $data3      = array();
            if ($field == 'id_proof') {
        ...
        $data[ 'Emessage' ]                     = '';
        $data[ 'Smessage' ]                     = '';
        $data[ 'header' ][ 'title' ]            = 'Host Profile';
        $data[ 'header' ][ 'metakeyword' ]      = 'Host Profile';
        $data[ 'header' ][ 'metadescription' ]  = 'Host Profile';
        $data[ 'footer' ][ 'script' ]           = $script;
        $data[ 'content' ]                      = $content;
        $data[ 'breadcrumb' ]                   = '
                                              <li><a href="'.$this->config->item( 'admin_url' ).'guider">Host List</a></li>
                                              <li class="active">Host Profile</li>';
        $this->template( $data );
    }
    public function edit()
    {
        ...
        $data[ 'Emessage' ]                     = '';
        $data[ 'Smessage' ]                     = '';
        $data[ 'header' ][ 'title' ]            = 'Edit Host';
        $data[ 'header' ][ 'metakeyword' ]      = 'Edit Host';
        $data[ 'header' ][ 'metadescription' ]  = 'Edit Host';
        $data[ 'footer' ][ 'script' ]           = $script;
        $data[ 'content' ]                      = $content;
        $data[ 'breadcrumb' ]                   = '
                                              <li><a href="'.$this->config->item( 'admin_url' ).'guider">Host List</a></li>
    ...
    public function payout_info()
    {
        ...
        $data[ 'Emessage' ]                     = '';
        $data[ 'Smessage' ]                     = '';
        $data[ 'header' ][ 'title' ]            = 'Host Payout';
        $data[ 'header' ][ 'metakeyword' ]      = 'Host Payout';
        $data[ 'header' ][ 'metadescription' ]  = 'Host Payout';
        $data[ 'footer' ][ 'script' ]           = $script;
        $data[ 'content' ]                      = $content;
        $data[ 'breadcrumb' ]                   = '
                                              <li><a href="'.$this->config->item( 'admin_url' ).'guider">Host List</a></li>
                                              <li class="active">Host Payout</li>';
        $this->template( $data );
    }
    ...
    public function add()
    {
        ...
        if($phone_number && $this->input->post( 'submit' ) == 'new_host'){
            ...
            if( $this->form_validation->run() == FALSE ) {
                ...
                $this->session->set_flashdata('successMSG', 'Host added successfully.');
        ...
        $data[ 'Emessage' ]                     = '';
        $data[ 'Smessage' ]                     = '';
        $data[ 'header' ][ 'title' ]            = 'Add Host';
        $data[ 'header' ][ 'metakeyword' ]      = 'Add Host';
        $data[ 'header' ][ 'metadescription' ]  = 'Add Host';
        $data[ 'content' ]                      = $content;
        $data[ 'footer' ][ 'script' ]           = '';
        $data[ 'breadcrumb' ]                   = '
                                                   <li><a href="'.$this->config->item( 'admin_url' ).'guider">Manage Host</a></li>
                                                   <li class="active">Add Host</li>';
        $this->template( $data );
    }
    function guiderStatus(){
        ...
        if($status == 1){
            ...
            $this->session->set_flashdata('successMSG', 'Guider Activate successfully.');
        }else{
            $this->session->set_flashdata('successMSG', 'Guider Deactivate successfully.');
        }
    }
    //Password Update
