            ...
            if( ( $_SERVER['REQUEST_METHOD'] == 'POST' ) && $this->verify_validate( $user_input ) ) {
                ...
                if ( $phoneExist ){
                    ...
                    if($phoneExist->status == 4){
                        $msg        = 'Your account has been successfully Recreated';
                ...
                $guiderInfo         = $this->Guiderapimodel->guiderPhoneExists( $phone_number );
    ...
    public function registerProfile() {
        ...
        if( $_SERVER['HTTP_ACCESS_TOKEN'] != API_ACCESS_TOKEN ) {
            ...
            if( ( $_SERVER['REQUEST_METHOD'] == 'POST' ) && $this->profile_validate( $user_input ) ) {
                ...
                if(preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $email)){ $data1['email'] = $email; }
                $this->Guiderapimodel->updateGuiderByUuid( $data1, $guider_id );
                $updateDevice   = $this->Guiderapimodel->updateDeviceInfo( $_SERVER, $guider_id );
                ...
                if($guiderInfo->verify_email == 0){
                    ...
                    $this->Guiderapimodel->updateGuiderByUuid( $data5, $guider_id );
                }
                $upload_path_url    = $this->config->item( 'upload_path_url' );
                $profileImgPath     = $upload_path_url.'g_profile/';
                $activityImgPath    = $upload_path_url.'g_activity/';
    ...
    public function updateBankInfo() {
        ...
        if( $_SERVER['HTTP_ACCESS_TOKEN'] != API_ACCESS_TOKEN ) {
            ...
            if( ( $_SERVER['REQUEST_METHOD'] == 'POST' ) && $this->bank_validate( $user_input ) ) {
                ...
                $result         = array(
                                        'response_code'     => SUCCESS_CODE, 
                                        'response_description' => 'Bank Information updated successfully.',
                                        'result'            => 'success',
                                        'is_profile_updated'=> $profile_updated,
                                        'is_bank_updated'   => $bank_updated,
                                        'is_service_updated'=> $service_updated,
    ...
    public function updateServiceInfo() {
        ...
        if( $_SERVER['HTTP_ACCESS_TOKEN'] != API_ACCESS_TOKEN ) {
            ...
            if( ( $_SERVER['REQUEST_METHOD'] == 'POST' ) && $this->service_validate( $user_input ) ) {
                ...
                $date_time_needed    = $user_input['date_time_needed'];
    ...
    public function getMyJourneys() {
        ...
        if( $_SERVER['HTTP_ACCESS_TOKEN'] != API_ACCESS_TOKEN ) {
            ...
            if( ( $_SERVER['REQUEST_METHOD'] == 'POST' ) && $this->myJourneyValidate( $user_input ) ) {
                ...
                $result      = $this->Guiderapimodel->getMyJourneyList( $page_total, $offset, $guider_id, $filter_type );
            } else {
                if( $_SERVER['REQUEST_METHOD'] != 'POST' ) {
                    $result = array('message' => 'Undefined Request Method', 'result' => 'error');
                } else if (isset($this->error['warning'])) {
    ...
    public function uploadproof() {
        ...
        if( $_SERVER['HTTP_ACCESS_TOKEN'] != API_ACCESS_TOKEN ){
            ...
            if( ( $_SERVER['REQUEST_METHOD'] == 'POST' ) && $this->validateproof( $_POST ) ) {
                ...
                $result         = array(
                                        'response_code'     => SUCCESS_CODE, 
                                        'response_description' => 'Identity Proof updated successfully.',
                                        'result'            => 'success',
                                        'is_profile_updated'=> $profile_updated,
                                        'is_bank_updated'   => $bank_updated,
                                        'is_service_updated'=> $service_updated,
