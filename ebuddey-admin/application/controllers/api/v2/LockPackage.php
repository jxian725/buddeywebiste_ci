        ...
        if( $_SERVER['HTTP_ACCESS_TOKEN'] != API_ACCESS_TOKEN ) {
            ...
            if( ( $_SERVER['REQUEST_METHOD'] == 'POST' ) && $this->lock_validate( $user_input ) ) {
                ...
                foreach ($packageIDs as $key => $package_id) {
                    ...
                    $partnerInfo = $this->Guiderapimodel->partnerInfo( $packageInfo->partner_id );
                    if($partnerInfo && $partnerInfo->required_license){
                        $licenseArr  = explode(',', $partnerInfo->required_license);
                        foreach ($licenseArr as $license_id) {
                            $licenseInfo = $this->Guiderapimodel->licenseInfo($license_id);
                            if(!$licenseInfo){ continue; }
                            if(!$this->Guiderapimodel->talentLicenseInfo($user_id, $license_id)){
                                if (!in_array(intval($license_id), $reqLicID)){
                                    $reqLicID[] = intval($license_id);
                                }
                                $eResMsg = 4;
                                $licErr  = 1;
                            }
                ...
                if($eResMsg == ''){
                    ...
                    if($eResMsg == 1){
                        ...
                        $result = array('response_code' => ERROR_CODE, 'response_description' => 'Please upload all required verification.', 'result' => 'error', 'license_ids' => $reqLicID, 'data' => array('error' => 1));
                    }
