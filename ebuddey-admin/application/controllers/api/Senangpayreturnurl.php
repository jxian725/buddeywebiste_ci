            ...
            if($hashed_string == urldecode($_GET['hash']))
            {
                if(urldecode($_GET['status_id']) == '1'){
                    ...
                    if($getISpayInfo){
                        ...
                        //BOOKING VIA WEB
                        }else if($getISpayInfo->paymentAppType == 'web_space_booking'){
                            $transaction_id = $_GET['transaction_id'];
                            $data777    = array(
                                                "transaction_id"        => $transaction_id,
                                                "senangpay_msg"         => urldecode($_GET['msg']),
                            ...
                            fclose($fWrite);
                            $this->session->set_flashdata('paySuccess', $transaction_id);
                            redirect( $this->config->item( 'front_url' ) . 'talent/buskerspod' );
                        //DONATION PAYMENT
                        }else if($getISpayInfo->paymentAppType == 'scan_payment'){
                            $data66     = array(
                                                "transaction_id"        => $transaction_id,
                                                "senangpay_msg"         => urldecode($_GET['msg']),
                                                "pay_updated"           => $createdon,
                ...
                //BOOKING FAILED
                    ...
                    if($getISpayInfo){
                        ...
                        if($getISpayInfo->paymentAppType == 'space_booking' || $getISpayInfo->paymentAppType == 'web_space_booking'){
                        ...
                        fclose($fWrite);
                        $this->session->set_flashdata('payError', $transaction_id);
                        if($getISpayInfo->paymentAppType == 'web_space_booking'){
                            redirect( $this->config->item( 'front_url' ) . 'talent/senangpay/failed' );
                        }else{
                            redirect( $this->config->item( 'admin_url' ) . 'api/senangpay/failed' );
                        }
                    }
