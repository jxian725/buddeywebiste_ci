...
class Partners extends CI_Controller {
    ...
    public function edit() {
	 ...
	 public function blocktalent() {
        ...
        $tplData[ 'partner_id' ]  = $partner_id;
        $tplData[ 'partnerInfo' ] = $partnerInfo;
		$tplData[ 'blockedtalentList' ] = $this->Partnermodel->getBlockedTalent($partner_id);
        $tplData[ 'partnerList' ] = $this->Partnermodel->partnerList();
        $tplData[ 'stateList' ]   = $this->Commonmodel->state_list( $country_id=132, 1 );
        $tplData[ 'licenseLists' ]= $this->Licensemodel->licenseLists();
		$tplData[ 'guiderInfo' ] =  $this->Guidermodel->guider_lists( false, 1 );
        $content    = $this->load->view( 'partner/block_talents', $tplData, true );
        $data[ 'navigation' ]                   = '';
        $data[ 'Emessage' ]                     = '';
        $data[ 'Smessage' ]                     = '';
        $data[ 'header' ][ 'title' ]            = 'Block Talent';
        $data[ 'header' ][ 'metakeyword' ]      = 'Block Talent';
        $data[ 'header' ][ 'metadescription' ]  = 'Block Talent';
        $data[ 'footer' ][ 'script' ]           = '';
        $data[ 'content' ]                      = $content;
        $data[ 'breadcrumb' ]                   = '<li class="active">Block Talent</li>';
        $this->template( $data );
    }
	function deleteBlockTalent() {
		$this->Partnermodel->deleteBlockTalent($this->input->post( 'partnerid' ) );
		echo "1";
	}
	function addBlockTalent() {
		
		$this->Partnermodel->deleteBlockTalent($this->input->post( 'partnerid' ) );
        $talents_id       = $this->input->post( 'talents_id' );
		$data = array();
        for ($i=0; $i < count($talents_id); $i++) { 
		
			$data['space_id']=   $this->input->post( 'partnerid' );
			$data['talent_id'] = $talents_id[$i]['value'];      
		$this->Partnermodel->addBlockTalent( $data );
		}
		echo "1";
