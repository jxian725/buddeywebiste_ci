                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th nowrap style="width:500px">User Name</th>
                                    <th>Email</th>
                                    <th>Phone No</th>
									...
									<th>Strength</th>
                                    <th>Status</th>
                                    <th nowrap style="width:500px">Action</th>
                                </tr>
                            </thead>
                            <tbody id="guider_lists">
                                ...
                                if($guider_lists) {
                                    foreach ( $guider_lists as $key => $value ) {
										...
										if(trim($value->strength)=="Good"){
											$strengthicon = '<a href="javascript:;" id='.$value->strength.' class="'.$value->strength_comment.'" onClick="return updatestrength('.$value->guider_id.',this.id,this.className);"  data-toggle="tooltip" title="Update Strength" class="btn btn-xs btn-success"> <i class="fa fa-thumbs-up"></i></a>';
										  }
										  else if(trim($value->strength)=="Alert"){
											$strengthicon = '<a href="javascript:;" id='.$value->strength.' class="'.$value->strength_comment.'" onClick="return updatestrength('.$value->guider_id.',this.id,this.className);"  data-toggle="tooltip" title="Update Strength" class="btn btn-xs btn-warning"> <i class="fa fa-thumbs-up"></i></a>';
										  }
										  else if(trim($value->strength)=="Suspended"){
											$strengthicon = '<a href="javascript:;" id='.$value->strength.' class="'.$value->strength_comment.'" onClick="return updatestrength('.$value->guider_id.',this.id,this.className);"  data-toggle="tooltip" title="Update Strength" class="btn btn-xs btn-danger"> <i class="fa fa-thumbs-up"></i></a>';
										  }
										else
										{
											  $strengthicon = '<a href="javascript:;" onClick="return addstrength('.$value->guider_id.');"  data-toggle="tooltip" title="Add Strength" class="btn btn-xs btn-primary"> <i class="fa fa-thumbs-up"></i></a>';
										}
                                        ?>
                                        <tr>
...
function addstrength(guider_id)
{
	
	$( '#myModal .modal-title' ).html( 'Add Strength' );  
    $( '#myModal .modal-body' ).html( 'Select Strength' );
	$( '#myModal .modal-body' ).append( '<br /><select name="strength" id="strength" class="form-control"><option value="">--Select--</option><option value="Good">Good</option><option value="Alert ">Alert </option><option value="Suspended">Suspended</option></select>');
	$( '#myModal .modal-body' ).append( 'Comment' );
	$( '#myModal .modal-body' ).append( '<br /><input type="text" placeholder="Comment" class="form-control" id="comment">');
    $( '#myModal .modal-body' ).append( '<br /><br /><button class="btn btn-info btn-sm" id="btnsubmit" data-dismiss="modal">Submit</button>&nbsp;&nbsp;<button aria-hidden="true" data-dismiss="modal" class="btn btn-danger btn-sm">Cancel</button>' );
    $( '#myModal' ).modal({ backdrop: 'static', keyboard: false }); 
    ...
    $( "#btnsubmit" ).click(function() {
		...
		if(strength=="")
		{
			 toastr.error( 'Guider Strength must be selected.','error' );
			 return false;
		}
		else
		{
        var data = { 'guider_id':guider_id,'strength':strength,'comment':comment }
        $.ajax({
          type: "POST",
          url: adminurl + 'guider/guiderStrength',
          data: data,
          success: function( data ) {
            toastr.success( 'Guider Strength Added Successfully.','Success' );
            setTimeout( function() {
              location.reload();
            }, 2000 );
          }
...
function updatestrength(guider_id,strength,strengthcomment)
{	
	
	$( '#myModal .modal-title' ).html( 'Update Strength' );
    
    $( '#myModal .modal-body' ).html( 'Select Strength' );
	$( '#myModal .modal-body' ).append( '<br /><select name="strength" id="strength" class="form-control"><option value='+strength+'>'+strength+'</option><option value="Good">Good</option><option value="Alert ">Alert </option><option value="Suspended">Suspended</option></select>');
	$( '#myModal .modal-body' ).append( 'Comment' );
	$( '#myModal .modal-body' ).append( '<br /><input type="text" placeholder="Comment" class="form-control" id="comment" value="'+strengthcomment+'">');
    $( '#myModal .modal-body' ).append( '<br /><br /><button class="btn btn-info btn-sm" id="btnsubmit" data-dismiss="modal" onclick="updatefun('+guider_id+')">Submit</button>&nbsp;&nbsp;<button aria-hidden="true" data-dismiss="modal" class="btn btn-danger btn-sm">Cancel</button>' );
	var code = {};
	$("select[name='strength'] > option").each(function () {
    if(code[this.text]) {
        $(this).remove();
    } else {
...
function updatefun(guider_id)
	{
        ...
        $.ajax({
          ...
          data: data,
          success: function( data ) {
            toastr.success( 'Guider Strength Updated Successfully.','Success' );
            setTimeout( function() {
              location.reload();
            }, 2000 );
          }
