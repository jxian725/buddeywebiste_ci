...
class Buskerspodmodel extends CI_Model {
    ...
    public function addRevokeBooking($data){
        $this->db->insert( 'revoked_event_booking', $data );
        return true; 
    }
    public function senangpayPaymentInfo($order_id){
        $this->db->select('*');
        $this->db->where( 'order_id', "$order_id" );
        $query = $this->db->get('senangpay_transaction');
        return $query->row();
    }
    public function updateSenangpayPayment( $data, $order_id ) {
        $this->db->where( 'order_id', "$order_id" );
        $this->db->update( 'senangpay_transaction', $data );
        return true;
    }
    //Update status (18-03-2020)
