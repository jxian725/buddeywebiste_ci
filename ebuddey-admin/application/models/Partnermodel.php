...
class Partnermodel extends CI_Model {
	...
	public function deleteBlockTalent($space_id){
		$this->db->where( 'space_id', $space_id);
		$this->db->delete( 'space_blacklist' );
		
	}
	public function addBlockTalent($data){
		
    	$table  = $this->db->dbprefix('space_blacklist');
		$this->db->insert( $table, $data );
		return true; 
	}
	public function getBlockedTalent($partner_id){
		$this->db->select('talent_id');
	    $this->db->where('space_id', $partner_id );
	    $this->db->from('space_blacklist');
		$query = $this->db->get();
	    $rowcount = $query->num_rows();
	    if( $rowcount > 0 ) {
	    	$result = $query->result();
	...
	public function checkPartnerEventExists($partner_id){
		$this->db->select('id');
	    $this->db->where('partner_id', $partner_id );
