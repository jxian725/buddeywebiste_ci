...
class Commonmodel extends CI_Model {
	...
	//Host total count
	function host_total_count($status) {
		$this->db->select('*');
	...
	function DPendingHostTotalCount() {
		$this->db->select('*');
	    $this->db->where( 'status', 0 );
	    $query = $this->db->get( 'guider' );
	    return $query->num_rows();
	...
	function ActiveBookingTotal_count() {
		$this->db->select('*');
	...
	function host_total_pending( $status) {
		$this->db->select('*');
	    $this->db->where( 'status', $status );
	    //Date Range
    ...
    //NEW FILE UPLOAD
    function newFileUpload( $filename, $folder, $rename='', $allowedTypes = false, $size = '', $width = '', $height = '' ) {
