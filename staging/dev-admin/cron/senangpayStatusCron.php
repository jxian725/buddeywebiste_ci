        ...
        if(!$response->data){
            ...
            if($payment_info){
                ...
                if($status == 'paid'){
                    ...
                    if($paymentAppType == 'space_booking'){
                        $update33   = 'UPDATE events 
                                       SET host_id = "'.$guiderID.'", paidStatus = 1, status = 3, bookedType = "APP", color = "#9c27b0", transactionID = "'.$transaction_id.'", paidDatetime = "'.$createdon.'"
                    ...
                    if($paymentAppType == 'space_booking'){
                        $update55   = 'UPDATE events 
                                       SET host_id = 0, paidStatus = 0, status = 1
                                       WHERE orderID = "'.$order_id.'"';
