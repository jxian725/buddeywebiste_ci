...
class Guiderapimodel extends CI_Model {
    ...
    public function updateGuiderId($guider_id){
        $g_id   = 'G-'.str_pad($guider_id, 5, '0', STR_PAD_LEFT);
    ...
    function getMyJourneyList( $limit=1000, $start=false, $guider_id=false, $filter_type=false ) {
        $data = array();
        ...
        if($filter_type != 1){
            $data2  = $this->getMySpaceBookingList($guider_id, $filter_type);
    ...
    public function getMySpaceBookingList( $guider_id, $status='' ){
        $data = array();
        $upload_path_url = $this->config->item( 'upload_path_url' );
        ...
        if( $query->num_rows() > 0 ) {
            ...
            foreach ( $spaceLists as $space ) {
                ...
                $spaceInfo = array(
                                ...
                                "additional_info"=> (($partnerInfo->address) ? $partnerInfo->address : ''),
