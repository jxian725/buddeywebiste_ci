...
class Host extends CI_Controller {
	...
	public function index() {
        ...
        if( $this->input->post( 'first_name' ) ) {
            ...
            $about_the_activity = $this->input->post( 'about_the_activity' );
            ...
            if(!$this->Commonmodel->phoneno2Verify($mobile_no)){
                ...
                $db_data2[ 'what_i_offer' ]      = $about_the_activity;
        ...
        $data[ 'Emessage' ]                     = '';
        $data[ 'Smessage' ]                     = '';
        $data[ 'header' ][ 'title' ]            = 'Host';
        $data[ 'header' ][ 'metakeyword' ]      = 'Host';
        $data[ 'header' ][ 'metadescription' ]  = 'Host';
    ...
    function view() {
        $activity_id    = base64_decode($this->input->get('activity_id'));
        ...
        $data_db[ 'skill_id' ]          = $skill_id;
        $data_db[ 'city_id' ]           = $city_id;
        $data_db[ 'guiderInfo' ]        = $activityInfo;
        $data_db[ 'activity_id' ]       = $activity_id;
        $data_db[ 'guider_id' ]         = $activityInfo->activity_guider_id;
        $data_db[ 'processing_fee' ]    = $processing_fee;
        $data_db[ 'comment_list' ]      = $this->Commonmodel->get_guider_comment_list( $activity_id );
        //$content     = $this->load->view( 'guider/guider_view', $data_db, true );
        $content     = $this->load->view( 'guider/host_view', $data_db, true );
        $data[ 'navigation' ]                   = '';
        $data[ 'Emessage' ]                     = '';
        $data[ 'Smessage' ]                     = '';
        $data[ 'header' ][ 'title' ]            = 'Host Information';
        $data[ 'header' ][ 'metakeyword' ]      = 'Host Information';
        $data[ 'header' ][ 'metadescription' ]  = 'Host Information';
        $data[ 'footer' ][ 'script' ]           = $script;
        $data[ 'content' ]                      = $content;
        $this->template( $data );
        return true;
    ...
    function postCommentValidate() {
        ...
        if( $this->form_validation->run() == FALSE ) {
            ...
            $guestImg   = $this->config->item( 'dir_url' ).'images/avatar.png';
            $html = '<div class="row">
