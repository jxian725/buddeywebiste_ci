...
<section class="blog_part section_padding" style="display: none;">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-xl-5">
                <div class="section_tittle text-center">
            ...
            <?php
            $access_token = 'IGQVJVUXVIZA0l6MnpNRHlIbk12a2N5cHFLZAU9GbDJxeUJRUEs4eTJiMC1iWE5mQzVSSlpzS3lWWDRPUVVIUmdhNzFqYkltMXU5ZAjZAfUi1YY2JpWmJBb2ZAuQWxhVDZAxa0RzVXF1emttT3E0eWxfZAkNGLQZDZD';
            $curl = curl_init();

            curl_setopt_array($curl, array(
              CURLOPT_URL => "https://graph.instagram.com/me/media?fields=id,caption&access_token=".$access_token,
              ...
              CURLOPT_HTTPHEADER => array(
                "Cookie: ig_did=BE557D0C-79EA-4BE4-AF28-A47B72154523; csrftoken=EUSkpbVGhZyCZ8wDAz5x8MqBxA2XGKy2; mid=Xu7_0wAEAAEkw7VhPWEZI04lYNrp"
              ),
            ));

            $response = curl_exec($curl);
            curl_close($curl);
            $dataLists = json_decode($response);
            echo '<pre>';
            print_r($dataLists);
            echo '</pre>';
            if($dataLists->data){
                echo '<div class="row new_insta_feed owl-carousel owl-theme" id="new_insta_feed">';
                foreach ($dataLists->data as $key => $value) {
                    $curl = curl_init();

                    curl_setopt_array($curl, array(
                      CURLOPT_URL => "https://graph.instagram.com/".$value->id."?fields=id,media_type,media_url,thumbnail_url,caption,username,timestamp&access_token=".$access_token,
                      CURLOPT_RETURNTRANSFER => true,
                      CURLOPT_ENCODING => "",
                      CURLOPT_MAXREDIRS => 10,
                      CURLOPT_TIMEOUT => 0,
                    ...
                    curl_close($curl);
                    $dataLists2 = json_decode($response2);
                    print_r($dataLists2);
                    if($dataLists2->error){ continue; }
                    
                    if($dataLists2->media_type == 'VIDEO'){
                        $image_url = $dataLists2->thumbnail_url;
                    }else{
                        $image_url = $dataLists2->media_url;
                    }
                    ?>
                    <div class="item">
                        <div class="single-home-blog">
                            <div class="team_member_social_icon">
                                <img class="Profile_img" src="<?php echo $dirUrl; ?>assets/img/instagram_img.jpg" style="height:auto;;width:10%;border-radius: 50%;display: -webkit-inline-box;">
                                <span class="insta_user"><a target="_blank" href="https://www.instagram.com/<?= $dataLists2->username; ?>"><?= $dataLists2->username; ?></a></span>
                                <div>
                                    <span class="right" style="padding-right: 5px;float: right;"><i class="ti-instagram"></i></span>
                                    <span class="left insta_date"><?= date('M j, Y', strtotime($dataLists2->timestamp)); ?></span>
                                </div>
                            </div>
                            <div class="card">
                                <img src="<?= $image_url; ?>" class="card-img-top" alt="" style="min-height: 300px;max-height: 300px;">
                                <div class="card-body">
                                    <div><ul>
                                        <li><i class="ti-heart"></i> 50</li>
                                        <li class="insta_share"><a href="https://www.instagram.com/<?= $dataLists2->username; ?>" target="_blank"<i class="ti-share"></i> Share</a></li>
                                    </ul></div>
                                    <div class="insta_text"><?= mb_strimwidth($value->caption, 0, 160, '...');; ?></div>
                                </div>
                            </div>
                        </div>
                    </div>
